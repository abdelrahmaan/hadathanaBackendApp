#!/usr/bin/env python3
"""
Script to add hadith samples with resolved names to ambiguous narrator pairs.
Each sample is a tuple: [hadith_text, resolved_name]
"""
...
def find_hadiths_for_pair(
    ...
    """
    Find sample hadith texts with resolved names where both ambiguous_name and student appear.

    Returns:
        List of [hadith_text, resolved_name] pairs
    """
    samples = []
        ...
        for chain in chains:
            ...
            if (ambiguous_in_original or ambiguous_in_resolved) and \
               (student_in_original or student_in_resolved):
                ...
                # Find the resolved name for the ambiguous narrator
                resolved_ambiguous = None
                for narrator in narrators:
                    orig = narrator.get('original_name', '')
                    resolved = narrator.get('name', '')

                    if ambiguous_name == orig or ambiguous_name in resolved:
                        resolved_ambiguous = resolved
                        break

                # Add as [hadith_text, resolved_name]
                samples.append([hadith_text, resolved_ambiguous or ""])
                break
...
def main():
    """Main function to add hadith samples with resolved names to ambiguous pairs."""
    ...
    for i, pair in enumerate(ambiguous_data['ambiguous_pairs']):
        ...
        # Find sample hadiths with resolved names
