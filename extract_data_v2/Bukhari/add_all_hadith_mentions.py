#!/usr/bin/env python3
"""
Script to add ALL hadith mentions for ambiguous narrator pairs.
Each sample is a tuple: [hadith_text, ""]
"""
...
def find_all_hadiths_for_pair(
    ambiguous_name: str,
    student: str,
    hadiths: List[Dict]
) -> List[List[str]]:
    """
    Find ALL hadith texts where both ambiguous_name and student appear.

    Returns:
        List of [hadith_text, ""] pairs
    """
    samples = []
...
def main():
    """Main function to add ALL hadith mentions to ambiguous pairs."""
    ...
    for i, pair in enumerate(ambiguous_data['ambiguous_pairs']):
        ...
        # Find ALL hadiths (not limited to 3)
        samples = find_all_hadiths_for_pair(ambiguous_name, student, hadiths)

        # Add samples to the pair data
        pair['hadith_samples'] = samples

        print(f"[{i+1}/{len(ambiguous_data['ambiguous_pairs'])}] {ambiguous_name} <- {student}: Found {len(samples)} hadith(s)")

        if (i + 1) % 20 == 0:
            print(f"\n=== Progress: {i + 1}/{len(ambiguous_data['ambiguous_pairs'])} pairs processed ===\n")
    ...
    total_hadiths = sum(len(p.get('hadith_samples', [])) for p in ambiguous_data['ambiguous_pairs'])
    ...
    print(f"Total hadiths found: {total_hadiths}")
