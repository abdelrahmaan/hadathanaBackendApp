...
User question
    │
    ▼
MongoDB history seeding (last 3 QA pairs from chat_sessions collection)
    │  history + current question
    ▼
LangChain Agent (LangGraph, tool-calling, stateless — no InMemorySaver)
...
| Durable memory | Agent is stateless (no `InMemorySaver`). On each request `session.messages[-6:]` is loaded from MongoDB and prepended to the agent input — history survives server restarts |
| Thread titles | Auto-generated by a plain `init_chat_model` singleton (not `create_agent`) running concurrently with streaming via `asyncio.create_task()` — zero added latency |
| Session store | MongoDB `chat_sessions_dev/prod` — single source of truth for history, full turn history persisted, owned by authenticated user |
